...
export default function Home() {
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      const location = projectData.location?.split(',')[0] || 'location';
      const filename = `Material-Analysis-Report_${projectType}_${location}_${timestamp}.pdf`;
      ...
      try {
        ...
        console.warn('PDF generation failed, trying print method:', pdfError);
        
        // Fallback to print method
        try {
          await printReport(pdfRef, filename.replace('.pdf', ''));
          alert("Print dialog opened. Please save as PDF from the print dialog.");
          return;
        } catch (printError) {
          console.warn('Print method failed, falling back to clipboard:', printError);
        }
      }
  ...
  const fallbackToClipboard = async () => {
    ...
    if (typeof materialReport === 'object' && !materialReport.rawText) {
      // JSON format - create a nicely formatted text version
      exportContent = `BUILDING MATERIAL ANALYSIS REPORT
Generated on: ${new Date().toLocaleDateString()}

=== EXECUTIVE SUMMARY ===
Total Cost: ${materialReport.executiveSummary?.totalCostEstimate || 'N/A'}
Timeline: ${materialReport.executiveSummary?.timelineOverview || 'N/A'}
Climate Suitability: ${materialReport.executiveSummary?.climateSuitability || 'N/A'}

Key Recommendations:
${materialReport.executiveSummary?.keyRecommendations?.map(rec => `• ${rec}`).join('\n') || 'N/A'}

=== COST ANALYSIS ===
${Object.entries(materialReport.costAnalysis?.materialCosts || {}).map(([key, value]) => `${key.charAt(0).toUpperCase() + key.slice(1)}: ${value}`).join('\n')}
Labor Costs: ${materialReport.costAnalysis?.laborCosts || 'N/A'}
Transportation: ${materialReport.costAnalysis?.transportationCosts || 'N/A'}
Contingency: ${materialReport.costAnalysis?.contingency || 'N/A'}
TOTAL PROJECT COST: ${materialReport.costAnalysis?.totalProjectCost || 'N/A'}

=== RECOMMENDED MATERIALS ===
${Object.entries(materialReport.recommendedMaterials || {}).map(([category, materials]) => `
${category.toUpperCase()} MATERIALS:
${materials.map(material => `  • ${material.name} (${material.brand || 'Generic'}) - ${material.costPerUnit || 'Price on request'}`).join('\n')}
`).join('')}

=== ENVIRONMENTAL IMPACT ===
Carbon Footprint: ${materialReport.environmentalImpact?.carbonFootprint || 'N/A'}
Energy Efficiency: ${materialReport.environmentalImpact?.energyEfficiency || 'N/A'}
Sustainability Features:
${materialReport.environmentalImpact?.sustainability?.map(item => `• ${item}`).join('\n') || 'N/A'}

=== DURABILITY & MAINTENANCE ===
Expected Lifespan: ${materialReport.durabilityMaintenance?.expectedLifespan || 'N/A'}
Maintenance Frequency: ${materialReport.durabilityMaintenance?.maintenanceFrequency || 'N/A'}

=== PROFESSIONAL RECOMMENDATIONS ===
${materialReport.professionalRecommendations?.topCombinations?.map((combo, idx) => `
${idx + 1}. ${combo.name} (${combo.cost})
   Materials: ${combo.materials?.join(', ')}
   Benefits: ${combo.benefits?.join(', ')}
`).join('') || 'N/A'}

---
Generated by AI-Powered Building Material Selector
Compliant with NBC 2016 and Indian Building Standards`;
    } else {
      // Raw text format
      exportContent = materialReport.rawText || materialReport;
    }
    
    await navigator.clipboard.writeText(exportContent);
    alert("PDF generation failed, but report has been copied to clipboard as backup");
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              {materialReport && (
                <button
                  ...
                  className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
                >
                  {isExporting ? (
                    <>
            ...
            <div className="min-h-96 max-h-[80vh]">
              {isLoading ? (
